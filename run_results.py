...
def plot_controller(individual, setpoints, sim_input, control_input, simulator):  # Controller fitness
    ...
    for example in range(run_time):  # For duration of run
        ...
        weakness += np.mean(np.fabs(simulator_out[:, 11] - setpoints[:, example]))  # Time variant simulation
...
if __name__ == "__main__":
    ...
    setpoints = mod.unsqueeze(np.array(get_setpoints(shape=1)), axis=0)


    controller_list = []

    individual = mod.unpickle('R_Reconfigurable_Controller/champ_controller')
